...
const rosenthalDayContentSource = readFileSync(new URL("../src/data/rosenthal/dayContent.js", import.meta.url), "utf8");
const rosenthalCharacterContentSource = readFileSync(new URL("../src/data/rosenthal/characterContent.js", import.meta.url), "utf8");
const rosenthalExplorationContentSource = readFileSync(new URL("../src/data/rosenthal/explorationContent.js", import.meta.url), "utf8");
const rosenthalFinaleContentSource = readFileSync(new URL("../src/data/rosenthal/finaleContent.js", import.meta.url), "utf8");
const rosenthalIntroContentSource = readFileSync(new URL("../src/data/rosenthal/introContent.js", import.meta.url), "utf8");
const tutorialContentSource = readFileSync(new URL("../src/data/tutorialContent.js", import.meta.url), "utf8");
const tutorialIntroContentSource = readFileSync(new URL("../src/data/tutorial/introContent.js", import.meta.url), "utf8");
const tutorialDayActionContentSource = readFileSync(new URL("../src/data/tutorial/dayActionContent.js", import.meta.url), "utf8");
const tutorialNightChoiceContentSource = readFileSync(new URL("../src/data/tutorial/nightChoiceContent.js", import.meta.url), "utf8");
const tutorialEndingContentSource = readFileSync(new URL("../src/data/tutorial/endingContent.js", import.meta.url), "utf8");
...
const systemMetaContentSource = readFileSync(new URL("../src/data/system/metaContent.js", import.meta.url), "utf8");
const systemRoleContentSource = readFileSync(new URL("../src/data/system/roleContent.js", import.meta.url), "utf8");
const systemMarkUnlockContentSource = readFileSync(new URL("../src/data/system/markUnlockContent.js", import.meta.url), "utf8");
const systemAffinityMarkContentSource = readFileSync(new URL("../src/data/system/affinityMarkContent.js", import.meta.url), "utf8");
const systemStandaloneMarkContentSource = readFileSync(new URL("../src/data/system/standaloneMarkContent.js", import.meta.url), "utf8");
const systemHiddenRuleContentSource = readFileSync(new URL("../src/data/system/hiddenRuleContent.js", import.meta.url), "utf8");
...
assert.equal(rosenthalDayContentSource.includes("export const DAY_ACTIONS"), true, "Rosenthal day edit source must keep DAY_ACTIONS in dayContent.js");
assert.equal(rosenthalCharacterContentSource.includes("export const CORE_NPCS"), true, "Rosenthal character edit source must keep NPC data in characterContent.js");
assert.equal(rosenthalExplorationContentSource.includes("export const DIRECTIONS"), true, "Rosenthal exploration edit source must keep directions in explorationContent.js");
assert.equal(rosenthalFinaleContentSource.includes("export const FINALES"), true, "Rosenthal finale edit source must keep finales in finaleContent.js");
assert.equal(rosenthalIntroContentSource.includes("export const PROLOGUE"), true, "Rosenthal intro edit source must keep prologue text in introContent.js");
assert.equal(rosenthalScriptContentSource.includes("export const DAY_ACTIONS"), false, "rosenthalScriptContent.js must not own editable day action content");
...
assert.equal(tutorialContentSource.includes("./tutorial/introContent.js"), true, "tutorialContent.js는 튜토리얼 intro edit source를 감싸야 함");
assert.equal(tutorialContentSource.includes("./tutorial/dayActionContent.js"), true, "tutorialContent.js는 튜토리얼 day action edit source를 감싸야 함");
assert.equal(tutorialContentSource.includes("./tutorial/nightChoiceContent.js"), true, "tutorialContent.js는 튜토리얼 night choice edit source를 감싸야 함");
assert.equal(tutorialContentSource.includes("./tutorial/endingContent.js"), true, "tutorialContent.js는 튜토리얼 ending edit source를 감싸야 함");
assert.equal(tutorialContentSource.includes("export const DAY_ACTIONS"), false, "tutorialContent.js는 낮 행동 본문을 직접 소유하지 않는다");
assert.equal(tutorialContentSource.includes("export const NIGHT_CHOICES"), false, "tutorialContent.js는 밤 선택지 본문을 직접 소유하지 않는다");
assert.equal(tutorialIntroContentSource.includes("export const PROLOGUE"), true, "튜토리얼 intro edit source는 프롤로그 메타를 가져야 함");
assert.equal(tutorialIntroContentSource.includes("export const DAY_OPENING_SCRIPT"), true, "튜토리얼 intro edit source는 낮 진입 대사를 가져야 함");
assert.equal(tutorialIntroContentSource.includes("export const DAY_INTERLUDES"), true, "튜토리얼 intro edit source는 낮 인터루드를 가져야 함");
assert.equal(tutorialIntroContentSource.includes("export const NIGHT_ENTRY_SCRIPT"), true, "튜토리얼 intro edit source는 밤 진입 대사를 가져야 함");
assert.equal(tutorialDayActionContentSource.includes("export const DAY_ACTIONS"), true, "튜토리얼 day action edit source는 DAY_ACTIONS를 가져야 함");
assert.equal(tutorialNightChoiceContentSource.includes("export const NIGHT_CHOICES"), true, "튜토리얼 night choice edit source는 NIGHT_CHOICES를 가져야 함");
assert.equal(tutorialEndingContentSource.includes("export const ENDINGS"), true, "튜토리얼 ending edit source는 ENDINGS를 가져야 함");
assert.equal(tutorialEndingContentSource.includes("export const WORKER_NAME_CHOICES"), true, "튜토리얼 ending edit source는 WORKER_NAME_CHOICES를 가져야 함");
assert.equal(tutorialEndingContentSource.includes("export const FORFEIT_RESULTS"), true, "튜토리얼 ending edit source는 FORFEIT_RESULTS를 가져야 함");
...
assert.equal(systemContentSource.includes("./system/metaContent.js"), true, "systemContent.js는 시스템 메타 edit source를 감싸야 함");
assert.equal(systemContentSource.includes("./system/roleContent.js"), true, "systemContent.js는 직업/칭호/패시브 edit source를 감싸야 함");
assert.equal(systemContentSource.includes("./system/markUnlockContent.js"), true, "systemContent.js는 표식 해금 edit source를 감싸야 함");
assert.equal(systemContentSource.includes("./system/affinityMarkContent.js"), true, "systemContent.js는 성향 표식 edit source를 감싸야 함");
assert.equal(systemContentSource.includes("./system/standaloneMarkContent.js"), true, "systemContent.js는 독립 표식 edit source를 감싸야 함");
assert.equal(systemContentSource.includes("./system/hiddenRuleContent.js"), true, "systemContent.js는 숨은 규칙 edit source를 감싸야 함");
assert.equal(systemContentSource.includes("export const RESOURCE_META"), false, "systemContent.js는 메타 본문을 직접 소유하지 않는다");
assert.equal(systemContentSource.includes("export const AFFINITY_MARK_GROUPS"), false, "systemContent.js는 표식 그룹 본문을 직접 소유하지 않는다");
assert.equal(systemContentSource.includes("export const HIDDEN_RUN_RULES"), false, "systemContent.js는 숨은 규칙 본문을 직접 소유하지 않는다");
assert.equal(systemMetaContentSource.includes("export const RESOURCE_META"), true, "시스템 메타 edit source는 RESOURCE_META를 가져야 함");
assert.equal(systemMetaContentSource.includes("export const HORROR_DERIVED_META"), true, "시스템 메타 edit source는 HORROR_DERIVED_META를 가져야 함");
assert.equal(systemRoleContentSource.includes("export const JOBS"), true, "시스템 role edit source는 JOBS를 가져야 함");
assert.equal(systemRoleContentSource.includes("export const TITLES"), true, "시스템 role edit source는 TITLES를 가져야 함");
assert.equal(systemRoleContentSource.includes("export const PASSIVES"), true, "시스템 role edit source는 PASSIVES를 가져야 함");
assert.equal(systemMarkUnlockContentSource.includes("export const MARK_LOADOUT_LIMIT"), true, "시스템 mark unlock edit source는 MARK_LOADOUT_LIMIT를 가져야 함");
assert.equal(systemMarkUnlockContentSource.includes("export const MARK_BRANCH_UNLOCKS"), true, "시스템 mark unlock edit source는 MARK_BRANCH_UNLOCKS를 가져야 함");
assert.equal(systemAffinityMarkContentSource.includes("export const AFFINITY_MARK_GROUPS"), true, "시스템 affinity mark edit source는 AFFINITY_MARK_GROUPS를 가져야 함");
assert.equal(systemStandaloneMarkContentSource.includes("export const STANDALONE_MARKS"), true, "시스템 standalone mark edit source는 STANDALONE_MARKS를 가져야 함");
assert.equal(systemStandaloneMarkContentSource.includes("export const LEGACY_STIGMA_MARK_MAP"), true, "시스템 standalone mark edit source는 LEGACY_STIGMA_MARK_MAP을 가져야 함");
assert.equal(systemHiddenRuleContentSource.includes("export const HIDDEN_RUN_RULES"), true, "시스템 hidden rule edit source는 HIDDEN_RUN_RULES를 가져야 함");
...
assert.equal(USER_PROLOGUE.text.filter((line) => line === "…….").length, 5);
assert.ok(USER_PROLOGUE.text.some((line) => line.includes("무언가 떠오르려 할 때")));
assert.ok(USER_PROLOGUE.text.some((line) => line.includes("가지런히 모은 손 등에 흉터")));
assert.ok(USER_PROLOGUE.text.some((line) => line.includes("아무것도 모른다는 사실")));
...
assert.ok(DAY_INTERLUDES[0].paragraphs.some((paragraph) => paragraph.includes("태양이 떠 있는 동안에는 괜찮을 것 같다")));
assert.ok(DAY_INTERLUDES[0].paragraphs.some((paragraph) => paragraph.includes("그렇게 믿는다")));
